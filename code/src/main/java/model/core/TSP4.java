    ...
    protected int bound(Integer currentNode, ArrayList<Integer> notSeen, int[][] cost, int[] duration) {
        ...
        for (Integer node1 : notSeen) {
            ...
            if (notSeen.indexOf(node1 + nbNodes / 2) == -1) {
                firstEnterEstimation = Math.min(firstEnterEstimation, cost[currentNode][node1]);
            }
            if (node1 > nbNodes / 2) {
                lastLeaveEstimation = Math.min(lastLeaveEstimation, cost[node1][0]);
            }
            costEstimation += (minEnter + minLeave) / 2;
            durationEstimation += duration[node1];
        }

        return costEstimation + durationEstimation + firstEnterEstimation / 2 + lastLeaveEstimation / 2;
    }
}
