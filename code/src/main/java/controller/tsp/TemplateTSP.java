...
public abstract class TemplateTSP implements TSP {
    ...
    public void searchSolution(int timeLimit, int nbNodes, int[][] cost,
                               int[] duration) {
        ...
        seen.add(0);
        branchAndBound(0, notSeen, seen, 0, cost, duration,
     ...
     * @param duration    duration[i] = duration to visit the i-th node, with
     *                   0 <= i < nbNodes
     * @return a lower bound of the cost
     */
    protected abstract int bound(Integer currentNode,
                                 ArrayList<Integer> notSeen, int[][] cost,
                                 int[] duration);
     ...
     * @param duration    duration[i] = duration to visit the i-th node, with
     *                   0 <= i < nbNodes
     * @return an iterator which can iterate the permutation of the nodes not
     * seen
     */
     ...
     */
    void branchAndBound(int currentNode, ArrayList<Integer> notSeen,
                        ArrayList<Integer> seen, int currentCost,
                        int[][] cost, int[] duration, long startTime,
                        int timeLimit) {
